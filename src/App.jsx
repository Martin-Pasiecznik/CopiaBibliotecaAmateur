...
const OnboardingModal = ({ user, onSave, darkMode, theme }) => {
  ...
  const modalStyle = {
    ...
    alignItems: 'center', zIndex: 3000, backdropFilter: 'blur(12px)'
  };

  const cardStyle = {
    backgroundColor: theme.bg,
    color: theme.textMain,
    padding: '40px', borderRadius: '28px', width: '90%', maxWidth: '420px',
...
/>
          ...
          {localAvatars.map((file, i) => {
            ...
            return (
              <img 
                ...
                style={{
                  width: '100%', aspectRatio: '1/1', borderRadius: '10px', cursor: 'pointer',
                  border: selectedPhoto === url ? `3px solid ${theme.accent}` : '2px solid transparent',
                  transform: selectedPhoto === url ? 'scale(1.1)' : 'scale(1)',
                  transition: 'all 0.2s ease', backgroundColor: theme.card, objectFit: 'cover'
                }}
              />
            );
          })}
        ...
        <div style={{ textAlign: 'left', marginBottom: '25px' }}>
          <label style={{ fontSize: '0.7rem', fontWeight: 800, color: theme.textMuted, display: 'block', marginBottom: '5px' }}>O SUBE TU PROPIA FOTO</label>
          <input 
            type="file" 
            accept="image/*"
        ...
        <button 
          ...
          style={{
            backgroundColor: theme.accent, color: darkMode ? '#000' : '#fff', border: 'none', padding: '16px',
            borderRadius: '14px', cursor: 'pointer', fontWeight: 800, width: '100%',
            fontSize: '1rem', boxShadow: `0 4px 15px ${theme.accent}44`
          }}
        >
          Guardar y Entrar
        </button>
...
function App() {
  ...
  const refreshBooks = useCallback(() => {
    setLoading(true);
    const fetchAllBooks = fetch('http://127.0.0.1:5001/api/books').then(res => res.json()).catch(() => []);
    const fetchRecent = fetch('http://127.0.0.1:5001/api/books/recently-updated').then(res => res.json()).catch(() => []);
    const fetchFeatured = fetch('http://127.0.0.1:5001/api/books/featured-random').then(res => res.json()).catch(() => []);
  ...
  return (
    <GoogleOAuthProvider clientId="750793668642-7apu45i7te8b8gibnrelnhjgqj7vg512.apps.googleusercontent.com">
      ...
      {showOnboarding && user && (
        <OnboardingModal user={user} onSave={handleSaveProfile} darkMode={darkMode} theme={theme} />
      )}
            ...
            <nav style={{ 
              ...
              <div style={{ display: 'flex', gap: '30px', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '20px' }}>
                  <Link to="/rankings" style={{ color: theme.textMain, textDecoration: 'none', fontSize: '0.85rem', fontWeight: 600, opacity: 0.7 }}>Rankings</Link>
                  <Link to="/search" style={{ color: theme.textMain, textDecoration: 'none', fontSize: '0.85rem', fontWeight: 600, opacity: 0.7 }}>Explorar</Link>
                  {user && (
                    <>
...
/>
                ...
                <button className="mode-toggle" onClick={() => setDarkMode(!darkMode)} style={{ background: 'none', border: 'none', fontSize: '1.2rem', cursor: 'pointer' }}>
                  {darkMode ? '☀️' : '🌙'}
                </button>
              </div>
            </nav>
        ...
        <div style={{ maxWidth: isReaderRoute ? '100%' : '1100px', margin: '0 auto', padding: isReaderRoute ? '0' : '0 20px' }}>
          <Routes>
            <Route path="/" element={
              <main style={{ paddingTop: '80px' }}>
                <header style={{ marginBottom: '100px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <p style={{ color: theme.accent, fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.4em', textTransform: 'uppercase', marginBottom: '24px' }}>Comunidad Literaria</p>
                  <h1 style={{ fontSize: 'clamp(2.2rem, 6vw, 4.2rem)', fontWeight: 300, color: theme.textMain }}>El lugar donde <span style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', color: theme.accent, fontWeight: 800 }}>todo Autor</span> tiene visibilidad.</h1>
                </header>

                <section style={{ marginBottom: '60px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
                    <h2 style={{ fontSize: '1.1rem', fontWeight: 800, textTransform: 'uppercase' }}>Destacados</h2>
                  </div>
...
<section style={{ marginBottom: '60px' }}>
  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
    <h2 style={{ fontSize: '1.1rem', fontWeight: 800, textTransform: 'uppercase' }}>Actualizados Recientemente</h2>
      ...
      <Link key={`recent-${book.id}`} to={`/book/${book.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
        <div className="recent-item" style={{ 
          ...
          <div style={{ overflow: 'hidden' }}>
            <h3 style={{ fontSize: '0.95rem', fontWeight: 700, margin: '0 0 4px 0', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {book.title}
            </h3>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '8px' }}>
              <span style={{ fontSize: '0.7rem', color: theme.accent, backgroundColor: `${theme.accent}15`, padding: '2px 8px', borderRadius: '4px', fontWeight: 700 }}>
